...
This package is a client library for Joseki that integrates with rdflib
<http://rdflib.net>, a Python/RDF toolkit.  It requires rdflib version 2.

Some examples of use are given.

py-ex-1     Use the "fetch" operation to find out
            what a knowledge base knows about a resource

py-ex-2     Get a whole model.

py-ex-3     Use a simple query language to find statements with a
...
py-ex-4     Find the book with a given title, then retrieve, using the
...
py-ex-5     RDQL query, returning a subgraph of the original
            knowledge base graph

py-ex-6     RDQL query, getting a result set in RDF as the result
